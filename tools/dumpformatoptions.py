...
CLANG_DIR = os.path.join(os.path.dirname(__file__), "../../llvm-project/clang")
...
class OptionsReader:
    ...
    def __file_path(self):
        return "file"#os.path.relpath(self.header.name)
...
def option_rep(optionname, optiondef):
    ...
    for c in configs:
        fragments.append("        " + (str(c).split("_", maxsplit=1)[1] if "_" in str(c) else str(c)))
    rep = "\n".join(fragments)
    return rep
...
if __name__ == '__main__':
    ...
    for opt in opts:
        yt = to_yaml_type(opt.type)
        if "List of" in yt:
            continue
        print("+ " + option_rep(opt.name, (opt.type, [x.config for x in opt.enum.values] if opt.enum else [])))#)opt.enum if opt.enum else [])))
        #print(str(opt))
